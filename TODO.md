...
Показывать tracked threads в основном сайдбаре: top 5 + "Показать все" (Show all).

Если picture spoiler, то делать blur в сайдбаре и в хедере.

В хедере показывать мб квадратную картинку треда. Также показывать квадратные картинки тредов в сайдбаре. При новых сообщениях: точка слева и количество новых комментариев справа.
...
В Application.didMount запускать обновляльщик отслеживаемых тредов: через интервал в 15 минут будет в секунду обновлять по одному треду, показывая спиннер в сайдбаре (и пересортировывать после каждого обновлённого треда). Писать время refreshedAt, addedAt, latestReadCommentDate (для измерения возможного рейтинга stale thread), latestUnreadCommentDate, firstUnreadCommentDate (для измерения newCommentsRate для возможного ранжирования в будущем), newCommentsCount, newRepliesCount. Dates - timestamps.

При прочтении непрочитанных сообщений в треде — пересортировывать список отслеживаемых тредов.
...
Показывать список избранных досок в основном сайдбаре (наверху).
...
Отслеживаемые треды и Уведомления — показывать в сайдбаре прямо. В мобильной версии — экспандить его по нажатию на эти кнопки, и тогда у них будет pushable state (как у списка досок), и при отжатии их будет скрываться sidebar.





мб выводить last_replies у 4chan (по середине — ссылка "Show all comments").
...
При ответе в тред сделать галку "Не подписываться на этот тред".
...
Можно сделать страницу "Лента" (/feed) как в социальных сетях, на которой будут перечислены "отслеживаемые" треды, упорядоченные по "latest bump date". Для каждого отслеживаемого треда записывать "latest bump date", обновляя его при обновлении "отслеживаемого треда". Для перехода в ленту можно сделать отдельный пункт меню с картинкой "списка", а мобильнму меню "показать доски" сменить иконку, например, на `/a/`.

Сохранять "уведомления" где-то в user data: объект comment, repliedToComment, thread subject. `notification: { originalComment, reply, thread }`. Список `userData.notifications` можно ограничивать по длине, например, в 50 уведомлений, и "роллить" его при поступлении новых уведомлений.
