...
Список тредов сделать можно без скругления, и первую картинку показывать слева сверху через `float: left`, а по нажатию на "Показать полностью" — показывать остальные картинки (кроме первой) в виде attachments. При этом показывать кнопку "Показать полностью" даже если весь текстовый контент уместился (мб назвать это как-то "Показать вложения"). Основную картинку делать нормального размера, вписанной в квадрат, где фон — изображение с фильтром `blur` и "negative margin".

На странице треда — убрать фон, и сделать всё белым. Мб уменьшить слегка ширину колонки, и сайдбара. Сдвинуть само содержимое немного вправо (текст).
