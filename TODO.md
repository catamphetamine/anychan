* Можно написать функцию собирания только текстового содержимого из `post.content`: `getPostText(post)`. Это мог бы быть рекурсивный проход по `post.conent`. В браузере такое можно делать используя DOM:
...
`.post__inline-quote` при этом как-то оборачивать в парные кавычки (`«»`).

* Можно показывать темы в "уменьшенном" виде: брать `getPostText(post)`, триммить получившуюся строку через `limitLength()`, при этом увеличив длину, и добавив приоритетный тримминг по переводу на новую строку (`"\n"`), а не по концу предложения, и выводить рядом кнопку-ссылку "Показать полностью" (как вконтакте). При этом будет показываться только "основная" картинка треда: слева, вписанная в квадрат (`<Picture fit="cover"/>`).

* Функцию `getPostText(post)` можно также приделать к `setInReplyToQuotes()` взамен простого подхода, используемого там.
