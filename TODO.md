...
В слайдшоу анимировать закрытие того кадра, с которого оно было открыто, даже если было листание какое-то.

Мб не работает hot reload.

Иногда чаны не возращают коды ошибок, а возвращают HTML страницы типа "502 Gateway ...". Можно как-то детектировать, чем является ответ сервера — строкой или JSON'ом, и считать ошибкой, если не JSON. Можно также ставить заголовок типа "accepts: application/json" — мб это отработает как ошибка в случае неправильного ответа.





Что насчёт средних экранов: показывать ли там кнопку меню сбоку, или же хедер.
...
Сделать закрывашку у сайдбара там же, где кнопка открытия.
...
Мб сделать анимацию нажатия боковой кнопки меню получше (мб фон, мб ещё что-то). Мб подкрутить дизайн этой кнопки вообще.
