...
Можно "откреплять" треды и не учитывать их "закреплённость" вообще.
Отказаться от концепции "закреплённых" тредов.

В телеграме это решается интересным способом: "закреплённость" тредов не учитывается при отображении списка тредов, однако наверху присутствует полоска-баннер, в которой кратко перечислены все "закреплённые" треды. Возможно, сделать что-то такое. Полоска вроде как закрываемая.

Если делать такую полоску, то в сайдбаре её делать смысла нет, а на странице доски — можно.
...
Посмотреть, работает ли постинг `sys.4chan.org` на "work-safe" досках типа /g/, где используется обычно `sys.4channel.org`.
...
В инструкции по деплою можно дописать, что хостится приложение как статика, и что все url-ы следует перенаправлять на index.html, или скопировать index.html в 404.html.

Publish the website. Main pages of "old" sites should redirect to the new website.

Show refresh threads list button in threads list header.

В сайдбаре добавить иконку человечка. Если "вошёл", то внутри человечка можно рисовать галочку.

Maybe split `comment.onContentChange()` function in `imageboard` into itself and `.updateReplies()`.
...
Добавить параметр `outputAreaWidthInCharacters` в `getPostText()`, и из него вычислять `NEW_LINE_COST`: таким образом будет адекватнее `penalty` за `\n`. Также можно исчислять соответствующее `penalty * 2` за разделитель между параграфами.
...
Можно добавить концепцию "bookmark"-ов (закладок): "Bookmark comment" / "Bookmark thread". Each bookmark would come with an autogenerated title and a small picture. The relevant `UserData` collections would be called "bookmarkedComments" and "bookmarkedThreads".
