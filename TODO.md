...
В режиме редактирования "избранных" досок — показывать <Autocomplete/> (по нажатию на "..." — загружает список `getAllBoards()`, если не `chan.allBoards`), отфильтрованный по ещё не добавленным. При выборе — добавлять в список "избранных" досок. Обновлять список "избранных" досок во всех вкладках при его изменении. Добавить SortableList и переписать список досок на `<tr/>` (сохранять при изменении порядка досок). Close the github issue.
