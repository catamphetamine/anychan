...
find new line
if (trimmed at new line > 0.7) {
	return trimmed at new line
}
find sentence end
if (trimmed at sentence end && trimmed at new line < 0.5 * trimmed at sentence end) {
	return trimmed at sentence end
}
if (trimmed at new line > 0.5) {
	return trimmed at new line
}
find whitespace
if (trimmed at whitespace && trimmed at new line < 0.5 * trimmed at whitespace) {
	return trimmed at whitespace
}
return trimmed at any

что там с коэффициентом 0.75 для trim preview at sentence end.
убрать не используемый код из generate preview.

get post text - делать, наверное, через preview, если есть limit, потому что иначе как учитывать "\n" и параграфы.
если через get preview, то как-то там пропускать quotes и post links и autogenerated quotes (и "\n" между ними).
альтернативно, можно было бы ограничивать autogenerated quote тремя строками, но лучше через get preview.


предпочитать тримминг по new line или paragraph, а не по sentence.

не учитывать penalty при решении, как сокращать: по концу предложения или прямо как есть.

если текста мало (в основном penalty), то понижать порог для mid-sentence trim.

перегенерировать preview (и цитаты поста, и preview сообщений с ними) при загрузке видео и твитов.



Сделать ограничение длины комментария (и не загружать youtube video и tweets, пока не развёрнуто).
