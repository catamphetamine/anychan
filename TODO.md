...
Проверить, что items не перерендериваются при скролле.

Добавить в lynxchan определение оригинала как .thumb.replace(/\/t_/, '/') + imagepng -> '.png', etc.
...
Если в отслеживаемом треде есть новые сообщения, то поднимать его наверх и показывать +N новых сообщений каким-нибудь цветом (оранжевым, например). Если среди новых сообщений есть ответы, то показывать их количество так же как +M (мб зелёным, или красным, или ещё каким-то цветом).

Во вкладке "Уведомления" — показывать ответы.
...
После удаления треда из списка отслеживаемых показывать "Тред удалён из списка. Отменить".
