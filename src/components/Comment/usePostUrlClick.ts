...
export default function usePostUrlClick({
	...
	comment: Comment,
	onPostUrlClick: (event: Event, comment: Comment) => void
}) {
	const [clickedPostUrl, setClickedPostUrl] = useState(false)
	...
	const onPostUrlClickHandler = useCallback((event: Event) => {
		if (onPostUrlClick) {
