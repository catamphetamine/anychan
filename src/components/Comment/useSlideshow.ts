import type { Attachment, AttachmentSlide } from '@/types'
...
export default function useSlideshow({ comment }: { comment: Post }) {
	const dispatch = useDispatch()

	// `<PostLink/>` doesn't provide the `options` argument
	// (and `imageElement` as part of it) when calling `onAttachmentClick()`.
	const onAttachmentClick = useCallback((attachment: AttachmentSlide, {
		imageElement,
		attachments
	}: {
		...
		if (i >= 0) {
			dispatch(openSlideshow(attachments as AttachmentSlide[], i, { imageElement }))
		} else {
