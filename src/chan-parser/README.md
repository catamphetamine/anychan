...
* Add an export for the chan in `./src/chan-parser/chan/index.js` (same as for existing chans).

If the chan runs on an already supported engine then specify the `parser` property in `index.json`. The new chan being added most likely has its own comment HTML syntax which could be a bit different from other chans. In such case, go to the engine's directory and edit `Parser.js` to use the set of "comment parser plugins" specific to this new chan (see other chan's comment parser plugins as an example). Otherwise, if it's a new engine:
