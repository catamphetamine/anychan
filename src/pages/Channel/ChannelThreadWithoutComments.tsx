import type { ItemState, ItemComponentProps } from './types.js'
...
export default function ChannelThreadWithoutComments({
	...
	const onExpandContentChange = useCallback((expandContent: ItemState['expandContent']) => {
		setState({
			...state,
			expandContent
		})
	...
	const setHidden = useCallback((hidden: ItemState['hidden']) => {
		setState({
			...state,
			hidden
		})
...
ChannelThreadWithoutComments.propTypes = {
	...
	// Unused.
	// Added this property here just to work around TypeScript error.
