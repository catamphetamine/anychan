...
import type { UserData, UserSettings, DataSource, Announcement, Locale } from '@/types'
	...
	try {
		...
		if (process.env.NODE_ENV === 'production') {
			startPollingAnnouncement(
				...
				(announcement: Announcement ) => dispatch(setAnnouncement(announcement)),
...
async function onError(error: Error & { status?: number }, {
	dataSource,
	dataSourceAlias,
	location
}: {
	...
	// if (process.env.NODE_ENV === 'production') {
		...
		window.location.href = `${basePath}${errorPageUrl}?offline=✓&url=${encodeURIComponent(basePath + location.pathname + location.search + location.hash)}`
