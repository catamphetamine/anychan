import type { DataSource, UserSettings, GetChannelsParameters, ChannelFromDataSource } from '../types/index.js'
...
export default async function getChannels({
...
} & Omit<GetChannelsParameters, 'proxyUrl'>) {
	const {
		channels: channels_,
		hasMoreChannels
	...
	let channels: PossiblyHiddenChannel[];
	if (all) {
		channels = channels_
	} else {
		channels = markHiddenChannels(channels_, { dataSource })
	}
...
interface GetChannelsResult {
	...
	channelsByPopularity?: ChannelFromDataSource[];
	channelsByCategory?: { category: string, channels: ChannelFromDataSource[] }[];
}

/**
 * Returns the list(s) of channels.
 */
function getChannelsResult(channels: PossiblyHiddenChannel[], { dataSource }: { dataSource: DataSource }) {
	const result: GetChannelsResult = {
		channels
	}
...
function groupChannelsByCategory(
	channels: ChannelFromDataSource[],
	categoriesOrder: DataSource['contentCategories'] = []
) {
	const categories = []
