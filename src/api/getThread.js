...
export default async function getThread({
	...
	const thread = await Chan({ censoredWords, messages, http }).getThread({
		boardId,
		threadId,
	...
	for (const comment of thread.comments) {
		...
		addOnContentChange(comment, (isParentCommentUpdate) => {
			...
			return generateQuotes(comment.content, {
		...
		// Modify `.parseContent()` so that it calls `.onContentChange()`.
		...
		// without the actual content.
			...
			if (comment.replies && expandReplies) {
				return comment.replies
					.map(reply => reply.content && reply.onContentChange(true) && reply.id)
					.filter(_ => _)
			}
			return []
		}
