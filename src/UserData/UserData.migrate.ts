import type { UserDataChunkDataEncoded, UserDataChunkMetadata, UserDataCollection, UserDataCollections, UserDataCollectionDataEncoded, UserDataCollectionName } from '@/types'
...
const SOURCES = ['storage', 'object'] as const
...
interface Parameters {
	...
	writeData(key: string, chunkDataEncoded: UserDataChunkDataEncoded): void;
	removeData(key: string): void;
	writeCollectionChunkData(collection: UserDataCollection, metadata: UserDataChunkMetadata, chunkDataEncoded: UserDataChunkDataEncoded): void;
...
export default function migrate({
	...
	if (version < 5) {
		let threadVotes: Record<string, Record<string, -1 | 1>>
	...
	for (const collectionName of Object.keys(collections)) {
		...
		if (COLLECTION_CHUNK_MIGRATIONS[collection.name]) {
			let migrationVersions = Object.keys(COLLECTION_CHUNK_MIGRATIONS[collection.name]).map(Number)
...
const COLLECTION_CHUNK_MIGRATIONS: Record<UserDataCollectionName, Record<string, (chunk: UserDataChunkDataEncoded, parameters: { metadata: UserDataChunkMetadata }) => void>> = {
	// [collection.name]: {
	// 	[version]: (chunk) => {
	// 		chunk.a = 'b'
	// 	}
...
// Migrations for version <= 5.
const COLLECTION_DATA_MIGRATIONS_LEGACY: Record<UserDataCollectionName, Record<string, (chunk: UserDataCollectionDataEncoded) => void>> = {
	subscribedThreads: {
		// Version 1.
		// Dec 24, 2019.
	...
	latestReadComments: {
		...
		'1': (data: Record<string, Record<string, object>>) => {
		...
		'3': (data: Record<string, Record<string, { t?: number, threadUpdatedAt?: number }>>) => {
			for (const channelId of Object.keys(data)) {
				for (const threadId of Object.keys(data[channelId])) {
		...
		// July, 2021.
		// Rename `i` -> `number`.
		'4': (data: Record<string, Record<string, { i?: number, number?: number }>>) => {
			for (const channelId of Object.keys(data)) {
				for (const threadId of Object.keys(data[channelId])) {
		...
		'5': (data: Record<string, Record<string, { id: number }>>) => {
			for (const channelId of Object.keys(data)) {
				for (const threadId of Object.keys(data[channelId])) {
					// Added this assignment to fix TypeScript error.
					const newData = data as unknown as Record<string, Record<string, number>>
					newData[channelId][threadId] = data[channelId][threadId].id
...
function migrateCollectionDataLegacy({
	...
	if (COLLECTION_DATA_MIGRATIONS_LEGACY[collection.name]) {
		let migrationVersions = Object.keys(COLLECTION_DATA_MIGRATIONS_LEGACY[collection.name]).map(Number)
		migrationVersions = migrationVersions.map(Number).sort((a, b) => a - b)
		for (const migrationVersion of migrationVersions) {
			if (version < migrationVersion) {
		...
		if (isCollectionTypeSplitByChannelAndThread(collection.type)) {
			...
			for (const channelId of Object.keys(data)) {
				for (const threadId of Object.keys(data[channelId])) {
					writeCollectionChunkData(
						collection,
						{ channelId, threadId: Number(threadId) },
						data[channelId][threadId]
					)
				}
			}
