...
//
export const subscribedThreads: UserDataCollection = {
	...
	schema: {
		...
		own: {
			type: 'boolean',
			description: 'Whether this therad is the user\'s own thread',
			required: false
		},
		// latestComments: {
		// 	arrayOf: 'number',
	...
	//
	match: (encoded, _) => encoded.id === _.id && encoded.channel.id === _.channel.id,
