...
const debug = (...args: any[]) => console.log(['UserDataCleaner'].concat(args))
const debugLock = (...args: any[]) => console.log(['UserDataCleaner.Lock'].concat(args))
...
export default class UserDataCleaner {
	private tabStatusWatcher: TabStatusWatcher
	private unusedThreadDataLifeTime: number
	private startDelayMax: number
	private timer: Timer
	...
	_clean = async () => {
		...
		if (lockResult instanceof LockFail) {
			...
			this.cancelled = true
			this.schedule(lockResult.retryAfter)
			return
		}
