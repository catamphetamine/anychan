import type { UserData, Channel, Thread, SubscribedThread, UserDataCollection } from '@/types'
...
export default function fixSubscribedThreadsData(
	...
	function getSubscribedThread(channelId: Channel['id'], threadId: Thread['id']) {
		return subscribedThreads.find((subscribedThread) => {
			return subscribedThread.channel.id === channelId && subscribedThread.id === threadId
		})
	}

	const subscribedThreadIdsByChannel =  getSubscribedThreadIdsByChannelMap({ subscribedThreads })
...
function getSubscribedThreadIdsByChannelMap({ subscribedThreads }: { subscribedThreads: SubscribedThread[] }) {
	const subscribedThreadIdsByChannel: Record<string, Array<SubscribedThread['id']>> = {}
...
// Fix `subscribedThreadsIndex` collection.
function fixSubscribedThreadsIndex({
	userData,
	log,
	subscribedThreadIdsByChannel
}: {
	userData: UserData,
	log: (...args: any[]) => void,
	subscribedThreadIdsByChannel: Record<string, Array<SubscribedThread['id']>>
}) {
	log('Index: Analyze: Start')

	let changed

	const subscribedThreadIdsByChannelData = userData.getCollectionData(subscribedThreadsIndex as UserDataCollection) || {}
	...
	if (changed) {
		return () => userData.replaceCollectionData(subscribedThreadsIndex as UserDataCollection, subscribedThreadIdsByChannelData)
	}
}

// Fix `subscribedThreadsStats` collection.
function fixSubscribedThreadsStats({
	userData,
	log,
	subscribedThreadIdsByChannel,
	getSubscribedThread
