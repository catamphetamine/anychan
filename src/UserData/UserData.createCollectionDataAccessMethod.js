...
export default function createMethod(op, {
	...
	let { isEqual, compare } = collection

	if (getUnderlyingCollectionType(collection.type) === 'list') {
		if (!isEqual) {
			isEqual = getListItemIsEqualFunction(collection.type)
		}
		if (!compare) {
			compare = getListItemCompareFunction(collection.type)
		}
		if (!isEqual) {
			throw new Error(`Collection "${collection.name}" must define an \`isEqual\` function`)
		}
	...
	return (...args) => {
		...
		const operation = (op) => {
			...
			// it might contain just the ones that're used in `isEqual()` comparison.
			//
			try {
			...
			return operations[op].apply(this, [
				{
					...
					// `isEqual()` is used in "list" collections in operations like
					...
					// ("same items" means "sameness" in terms of `collection.isEqual()`).
		...
		const write = (value) => storage.set(key, value)
...
function getMetadata(args, collection, op) {
