import type { UserDataCollections, UserDataJson } from '@/types';

import migrateUserData from './UserData.migrate.js'
import { getCollectionDataObjectChunks } from './collection.utility.js'

export default function migrateUserDataObject(
	data: UserDataJson,
	{ collections, VERSION }: { collections: UserDataCollections, VERSION: number }
) {
	const version = data.version || 0
	if (version < VERSION) {
		for (const collectionName of Object.keys(data) as Array<keyof typeof data>) {
