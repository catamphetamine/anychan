import type { UserData, Channel, Thread, UserDataCollection } from '@/types'
...
export default function clearUnusedThreadsData({
	...
	userData: UserData,
	unusedThreadDataLifeTime: number,
	timer: Timer,
	log?: (...args: any[]) => void
}) {
	const _log = log
	...
	const availableThreadsByChannel = userData.getCollectionData(threads as UserDataCollection) || {}
	...
	function isTimeToClearArchivedThreadData(
		channelId: Channel['id'],
		threadId: Thread['id'],
		unusedThreadDataLifeTime: number
	) {
		...
		if (isCollectionTypeSplitByChannelAndThread(collection.type)) {
			...
			if (collection.type === 'channels-threads' || collection.type === 'channels-thread-data') {
				...
				for (const dataChunk of dataChunks) {
					...
					if (availableThreadsByChannel[channelId]) {
						function shouldKeepThreadData(threadId: Thread['id']) {
						...
						if (collection.type === 'channels-threads') {
							newData = newData.filter((threadId: Thread['id']) => shouldKeepThreadData(threadId))
