...
describe('UserData/onThreadsList', () => {
	it('should move threads to archive', () => {
		...
		onThreadsList('b', [], { now: 3000, userData })
		...
		expectToEqual(storage.getData(), {
			...
			'archive.hiddenThreads': {
				a: [100]
			},
			'archive.latestReadComments': {
				a: {
					'100': 101
				}
			},
		...
		expectToEqual(
			userData.getCommentVote('a', 100, 101),
			undefined
		)
		...
		expectToEqual(
			userData.getCommentVote('a', 500, 501),
			undefined
		)
	})
})