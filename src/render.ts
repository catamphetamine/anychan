import type { DataSource, Location, State, Action, LoadContext } from '@/types'
import type { Settings } from 'react-pages'
...
export default async function({
	...
	// @ts-expect-error
	const reactPagesSettings: Settings<State, Action, LoadContext> = getReactPagesConfig({
		dataSource,
		dataSourceAlias
	})

	const { enableHotReload } = await render(reactPagesSettings, {
		...
		onPageRendered({ location, params, context, dispatch, useSelector }) {
