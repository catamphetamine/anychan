import type { CachedStorageOptions } from 'web-browser-storage/cache'
...
export default class CachedStorage_ extends CachedStorage {
	constructor({
		...
		getLocale,
		...rest
	}: Omit<CachedStorageOptions<any>, 'flushDelay' | 'storage' | 'matchesPattern'> & {
		dispatch?: Dispatch,
		getLocale?: () => string
	}) {
		super({
			...rest,
			flushDelay: 30 * 1000,
			storage: new Storage({ dispatch, getLocale }),
