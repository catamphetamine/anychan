import type { UserDataCollection, UserDataCollectionName, UserSettings } from '@/types'
...
export default function migrate({
	...
	collections: Record<UserDataCollectionName, UserDataCollection>
	storage?: StorageType
	dispatch: Dispatch,
...
export function requiresMigration({ storage }: { storage?: StorageType } = {}) {
	// Initialize storage.
	if (!storage) {
		storage = new Storage()
	}
...
function migrate_({ version }: { version: string }, {
	collections,
	storage,
	log = debug
}: {
...
function isEarlierThan(version1: string, version2: string) {
