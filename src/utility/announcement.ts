import type { Announcement, UserData } from '@/types'

import { LockSuccess, LockFail } from 'web-browser-tab'
...
const debug = (...args: any[]) => console.log(['Announcement Refresh'].concat(args))
...
export function startPollingAnnouncement(
	url: string,
	showAnnouncement: (announcement: Announcement) => void,
	refreshInterval: number,
	{ userData }: { userData: UserData }
) {
	function retryAfter(delay: number) {
		// Randomize announcement refresh delay between different tabs
		// so that multiple tabs don't start refreshing it simultaneously.
	...
	async function pollAnnouncement() {
		...
		if (lockResult instanceof LockFail) {
			return retryAfter(lockResult.retryAfter)
		}
...
function onAnnouncement(announcement: Announcement, {
	userData,
	showAnnouncement
}: {
	userData: UserData,
 ...
 */
export async function fetchAnnouncement(url: string) {
	try {
...
export function markAnnouncementAsRead({ userData }: { userData: UserData }) {
...
function validateAnnouncement(json: Announcement) {
