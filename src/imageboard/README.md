...
* `attachmentThumbnailUrl` — (required for `"4chan"` engine) Same as `attachmentUrl` but for the thumbnail.
...
* `engine` — (optional) In case of accessing a chan that isn't supported out-of-the-box (`./src/imageboard/chan`) but runs on an engine that is supported out-of-the-box (`./src/imageboard/engine`) the `engine` object parameter can be passed to tell the library to use a specific engine from `./src/imageboard/engine` directory for accessing the chan. Example: `"4chan"`.

Available `options`:

* `request(method: string, url: string, parameters: object?): Promise` — (required) This function is used for sending HTTP requests to chan API. Example: `request("GET", "https://8ch.net/boards.json")`.
...
### Parser.getThreads({ boardId })

Returns a list of [Threads](#thread).

### Parser.getThread({ boardId, threadId })
...
```js
{
	...
	board: {
		...
		id: string,
		// Board title.
		// Some chans (like `2ch.hk`) provide it,
		// but most chans don't.
		title: string?
	},
	...
	// `2ch.hk` doesn't specify the limits in `/boards.json` API response.
	// Instead, it returns the limits as part of "get threads" and
	// "get thread comments" API responses.
	// `boardSettings` is only present in `2ch.hk` `Thread` objects.
	boardSettings: {
		...
		// Whether this board allows "badges" (like country flags but not country flags)
