...
* `request(method: string, url: string, parameters: object?)` — (required) This function is used for sending HTTP requests to chan API. Example: `request("GET", "https://8ch.net/boards.json")`.
...
### Parser.parseBoards(apiResponse, { hideBoardCategories })
