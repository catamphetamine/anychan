...
```js
{
  ...
  "enable_dices": 0, // Возможно, некий "ролл". Хз.
  "enable_flags": 0, // Показываются ли флаги у постов на данной доске.
  "enable_icons": 0, // Разрешены ли "значки" у постов на данной доске.
  ...
  "enable_shield": 0, // Возможно, включён ли какой-нибудь спам-лист, или что-то типа того.
  ...
  "enable_trips": 0, // Разрешены ли "трип-коды" при отправке сообщений на данной доске.
...
На некоторых досках (например, `/po/`) при отправке сообщения можно выбрать значок, который будет прикреплён к отправляемому сообщению (ID значка — число). ID значка можно взять из   Пример:
